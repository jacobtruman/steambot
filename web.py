...
def play_game(session, update_check=7):
    ...
    current = get_user_info(session, update_check)
    if current != -1:
        print("Leaving current planet")
        leave_game(session, current, update_check)
    print("Finding a planet and zone")
    zone_position, planet_id, planet_name, difficulty = get_zone(session)
    join_planet(session, planet_id, planet_name, update_check)
    while 1:
        join_zone(session, zone_position, difficulty, update_check)
        print("Sleeping for 1 minute 50 seconds")
        sleep(110)
        report_score(session, difficulty, update_check)
